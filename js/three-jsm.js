  ...
  this.pan = function (deltaX, deltaY) {
    ...
    if (scope.object instanceof THREE.PerspectiveCamera) {
      ...
      if (scope.object instanceof THREE.OrthographicCamera) {
        ...
        APPwarn('WARNING: OrbitControls.js encountered an unknown camera type - pan disabled.')
      }
    }
  }
  this.dollyIn = function (dollyScale) {
    ...
    if (scope.object instanceof THREE.PerspectiveCamera) {
      ...
      if (scope.object instanceof THREE.OrthographicCamera) {
        ...
        APPwarn('WARNING: OrbitControls.js encountered an unknown camera type - dolly/zoom disabled.')
      }
    }
  }
...
THREE.STLLoader.prototype = {
  ...
  parseBinary: function (data) {
    ...
    for (var index = 0; index < 80 - 10; index++) {
      if (
        reader.getUint32(index, false) == 1129270351 &&
        reader.getUint8(index + 4) == 82 &&
  ...
  DataView.prototype = {
    _getCharCodes: function (buffer, start, length) {
      ...
      for (var i = start; i < end; i++) {
        codes.push(buffer.charCodeAt(i) & 255)
      }
      return codes
    },
    ...
    getFloat64: function (byteOffset, littleEndian) {
      var b = this._getBytes(8, byteOffset, littleEndian),
        sign = 1 - 2 * (b[7] >> 7),
        exponent = ((((b[7] << 1) & 255) << 3) | (b[6] >> 4)) - ((1 << 10) - 1),
        mantissa =
          (b[6] & 15) * Math.pow(2, 48) +
          b[5] * Math.pow(2, 40) +
          b[4] * Math.pow(2, 32) +
          b[3] * Math.pow(2, 24) +
          b[2] * Math.pow(2, 16) +
